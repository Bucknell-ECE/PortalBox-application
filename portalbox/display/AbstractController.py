...
_BAD_COLOR_VALUE_ERROR_MSG = (
    "Colors must be a bytes object of "
    "length three (3) specifing the r, g and b "
    "components of the color in that order"
)
_BAD_DURATION_VALUE_ERROR_MSG = "duration must be an integer > 0"
_BAD_FLASHES_VALUE_ERROR_MSG = "flashes must be an integer"
_BAD_BACKPIXELS_VALUE_ERROR_MSG = "backpixels must be an integer > 0"
_BAD_CENTER_VALUE_ERROR_MSG = "center must be a non-negative integer"
_BAD_DIRECTION_VALUE_ERROR_MSG = "dir_down must be 0 or 1"
...
class AbstractController:
    ...
    def set_display_color_wipe(self, color, duration, dir_down):
        """
        ...
        if type(dir_down) is not int or dir_down < 0 or dir_down > 1:
            raise ValueError(_BAD_DIRECTION_VALUE_ERROR_MSG)
    ...
    def scroll_display(
        self, flash_color, duration, back_pixels, dir_down, center
    ):
        """Scroll color across all display pixels forever."""
        if type(flash_color) is not bytes or 3 != len(flash_color):
            raise ValueError(_BAD_COLOR_VALUE_ERROR_MSG)
        ...
        if type(back_pixels) is not int or back_pixels < 1:
            raise ValueError(_BAD_BACKPIXELS_VALUE_ERROR_MSG)

        if center != 0 and back_pixels > (center - 2):
            raise ValueError(_BAD_BACKPIXELS_VALUE_ERROR_MSG)
        ...
        if type(center) is not int or center < 0:
            raise ValueError(_BAD_CENTER_VALUE_ERROR_MSG)
